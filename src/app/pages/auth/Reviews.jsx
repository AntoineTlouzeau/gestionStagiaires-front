...
import ReviewListTable from '../../components/Meeting/Review/Review_list/ReviewListTable'; 

import {
    URL_BACK_GET_ALL_MANAGERS,
    URL_BACK_GET_ALL_REVIEWS,
...
function Reviews() {
    ...
    // New review states (for the modal)
    ...
    // Fetch reviews data whenever the filters change
    useEffect(() => {
        ...
        const fetchReviews = async () => {
            try {
                const response = await apiBackEnd.get(URL_BACK_GET_ALL_REVIEWS, {
                    params: {
                        size: size,
    ...
    // Get all unique meeting dates from reviews array in sorted order
    const uniqueDate = useMemo(() => [...new Set(reviews.map((review) => review.meetingDate).sort((a, b) => a.localeCompare(b)))], [reviews]);
    ...
    if (loading) {
        return (
            <div>
                <h3 className="principalTitle">Gestion des reviews</h3>
                <Loader />
            </div>
        );
    }

    // Create a variable to hold the props for ReviewListTableProps
    const ReviewListTableProps = {
        error: error,
    ...
    return (
        <div>
            <h3 className="principalTitle m-5">Gestion des reviews</h3>
            <div className="container mx-auto px-4">
                <ReviewListTable {...ReviewListTableProps} />
