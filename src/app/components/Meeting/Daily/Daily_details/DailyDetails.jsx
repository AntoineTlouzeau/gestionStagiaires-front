...
import DetailsComment from '../../../generic/DetailsComment';
...
const DailyDetails = () => {
    ...
    useEffect(() => {
        ...
        const fetchDataDaily = async () => {
            try {   
                const dataDaily = await apiBackEnd.get(URL_BACK_GET_ONE_MEETING.replace(':meetingId', meetingId), { signal });
                if (dataDaily.status === StatusCodes.OK) {
                    setDaily(dataDaily.data);
                    setLoading(false);
                    console.log(daily);
                }
            } catch (error) {
    ...
    if (loading) {
        return (
            <div>
                <h3 className="principalTitle">Détails d'un daily</h3>
                <Loader />
            </div>
        );
    }
    return (
        <>
            <h3 className="principalTitle">Détails du daily du {formattedSmallDate(daily.meetingDate)}</h3>
                ...
                <div className='reviewDetails bg-white dark:bg-background-dark rounded-xl p-5 w-full mb-4'>
                    <div className="flex flex-col-reverse md:flex-row">
                        <div className='w-full md:w-2/3 mb-4 lg:mr-6'>
                            ...
                            <div className="textDefault">{daily.location}</div>
                            <table className='w-full mt-6 text-left'>
                                ...
                                {daily.managersMeeting && daily.managersMeeting.map((item, index) => (
                                ...
                                {daily.internsMeeting && daily.internsMeeting.map((item, index) => {
                                    let presenceText = "";
                                    let presenceClass="";
                    ...
                    <div>                        
                        <div className="secondaryTitle mt-4">Commentaire du daily :</div>                        
                        <div className="textDefault"><DetailsComment text={daily.comment} /></div>
                    </div>
                </div>
            </div>
        </>
